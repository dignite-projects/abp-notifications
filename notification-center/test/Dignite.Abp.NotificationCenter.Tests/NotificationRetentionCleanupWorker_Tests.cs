...
/// <summary>
/// Exercises the retention worker's lock-contention, cancellation, and validation mechanics against a bare
/// service collection and a fake distributed lock. Provider-independent (no EF/Mongo store involved), so this
/// runs once instead of once per <see cref="NotificationStore"/> provider.
/// </summary>
public class NotificationRetentionCleanupWorker_Tests
{
    [Fact]
    ...
    public async Task Two_competing_retention_cycles_run_only_one_cleanup()
    {
        ...
        async Task WaitForReleaseAsync(CancellationToken cancellationToken)
        {
    ...
    {
        var attempt = 0;
        Task CleanupAsync(CancellationToken _)
        {

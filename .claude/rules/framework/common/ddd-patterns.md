...
**Encapsulation is key**: Protect entity state by using private/protected setters and exposing
...
- `AddDistributedEvent()` - Handled asynchronously, use ETOs (Event Transfer Objects). This repo's
  own cross-cutting event is `RealTimeNotifyEto` — see `notifications-invariants.md`.
...
- **Generic repository** (`IRepository<T, TKey>`): Sufficient for simple CRUD operations, and for
  ad hoc multi-condition queries written where they're consumed (see "In this repo" below).
- **Custom repository**: Only when you need query methods reused across multiple call sites.
...
- Use `*Manager` suffix naming (this repo: `NotificationDefinitionManager`, `UserNotificationManager`,
...
- **Check the DI lifetime of anything you inject** — see `notifications-invariants.md` §2 before
  marking a manager `ISingletonDependency`
...
The three `NotificationCenter.Domain` aggregates (`Notification`, `UserNotification`,
`NotificationSubscription`) deviate slightly from the generic template above in ways that are
**intentional for this repo — follow them, don't "fix" them back to the generic pattern**:

- They inherit `BasicAggregateRoot<Guid>` (not `AggregateRoot<Guid>` or `AuditedAggregateRoot<Guid>`)
  and implement `IMultiTenant` **explicitly** (`public virtual Guid? TenantId { get; protected set; }`)
  rather than relying on a richer built-in base class — `CreationTime` is likewise a plain modeled
  property, not ABP's audited-entity convention.
- Setters are `protected` (not `private`), since these entities live in the same project/assembly
  as the code that constructs them but the pattern is otherwise the same: constructor +
  behavior method (e.g. `UserNotification.SetState(...)`) rather than public setters.
- **No custom repository interfaces exist for any of the three aggregates.** All querying
  (including multi-field filters like "this user's unread notifications since date X") is written
  directly against the generic `IRepository<T, Guid>` inside the `INotificationStore`
  implementation — see `framework/data/ef-core.md`. Follow this for new queries against these
  aggregates; only reach for a custom repository interface for a genuinely new aggregate that
  needs the same query from multiple call sites.
